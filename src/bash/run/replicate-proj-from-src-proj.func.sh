...
# TGT_PATH=/opt/org/org-app/org-app-utl bash /opt/bas/bas-wpb/bas-wpb-utl/run -a do_replicate_proj_from_bas
...
do_replicate_proj_from_src_proj() {
  ...
  do_require_var SRC_ORG "${SRC_ORG:-}"   # Ensure SRC_ORG is defined
  do_require_var SRC_APP "${SRC_APP:-}"   # Ensure SRC_APP is defined
  ...
  BAS_PATH="/opt/bas/bas-wpb/bas-wpb-${PROJECT_TYPE}"
